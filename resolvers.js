...
const defaultStreamLocation = (id) => {
  const sourceBuckets = process.env.sourceBuckets.split(',');
  ...
  // Determine the bucket based on some logic. For example, the first letter of the id is odd or even. 
  const bucketIndex = id.charCodeAt(0) % sourceBuckets.length; // Example logic to choose a bucket
  const sourceBucket = sourceBuckets[bucketIndex];
